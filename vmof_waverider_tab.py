...
from waverider_generator.vmof_generator import VMOFWaverider
...
class VMOFCanvas3D(FigureCanvas):
    ...
    def plot_waverider(self, wr, half_only=False, show_upper=True,
        ...
        ma_min = getattr(wr, 'M_inf_min', getattr(wr, 'Ma_min', 0))
        ma_max = getattr(wr, 'M_inf_max', getattr(wr, 'Ma_max', 0))
    ...
    def _draw_info_panel(self, wr, title='VMOF WAVERIDER'):
        ma_min = getattr(wr, 'M_inf_min', getattr(wr, 'Ma_min', 0))
        ma_max = getattr(wr, 'M_inf_max', getattr(wr, 'Ma_max', 0))
        beta = getattr(wr, 'beta_deg', 0)
...
class VMOFWaveriderTab(QWidget):
    ...
    def _init_ui(self):
        ...
        # Generate button
        self.generate_btn = QPushButton("Generate VMOF Waverider")
    ...
    def _create_geometry_group(self):
        ...
        self.height_spin.setToolTip("Vehicle height (base plane vertical extent).")
    ...
    def _create_oc_group(self):
        group = QGroupBox("OC Planform Parameters")
        layout = QGridLayout()
        tips = [
        ...
        for i, (name, default, lo, hi, step) in enumerate([
            ("X1 (Flat Region):", 0.0, 0.0, 0.6, 0.01),
            ("X2 (LE Height):", 0.2, 0.0, 5.0, 0.01),
            ("X3 (Upper Surface):", 0.5, 0.0, 1.0, 0.01),
            ("X4 (Upper Surface):", 0.5, 0.0, 1.0, 0.01),
        ]):
        ...
        # Connect spins to update constraint hints
        for spin in [self.x1_spin, self.x2_spin, self.height_spin, self.width_spin]:
            spin.valueChanged.connect(self._update_constraint_hints)
    ...
    def generate_waverider(self):
        try:
            ...
            # Compute fillet compensation — scale height to lengthen vehicle
            ...
            if r_mm > 0.01:
                ...
                # OC length ∝ height, so scale height (and width) proportionally
                # First estimate length from a quick OC build
                from waverider_generator.generator import waverider as _OCW
                _tmp = _OCW(ma_max, beta, height, width,
                            [self.x1_spin.value(), self.x2_spin.value(),
                             self.x3_spin.value(), self.x4_spin.value()],
                            n_upper_surface=20, n_shockwave=20,
                            n_planes=5, n_streamwise=5)
                L_nom = _tmp.length
                scale = (L_nom + dL) / L_nom
                height *= scale
                width *= scale

            self.waverider = VMOFWaverider(
                M_inf_min=self.ma_min_spin.value(),
                M_inf_max=self.ma_max_spin.value(),
                ...
                dp=[self.x1_spin.value(), self.x2_spin.value(),
                    self.x3_spin.value(), self.x4_spin.value()],
                direction=direction,
                n_planes=self.n_points_spin.value(),
                n_streamwise=self.n_stream_spin.value(),
            )

            # Generate Ma_min and Ma_max reference waveriders (same compensated size)
            try:
                self._ma_min_ref = VMOFWaverider(
                    M_inf_min=self.ma_min_spin.value(),
                    M_inf_max=self.ma_min_spin.value(),
                    beta_deg=self.beta_spin.value(),
                    height=height,
                    width=width,
                    dp=[self.x1_spin.value(), self.x2_spin.value(),
            ...
            try:
                self._ma_max_ref = VMOFWaverider(
                    M_inf_min=self.ma_max_spin.value(),
                    M_inf_max=self.ma_max_spin.value(),
                    beta_deg=self.beta_spin.value(),
                    height=height,
                    width=width,
    ...
    def _update_constraint_hints(self):
        ...
        if oc_valid:
            ...
            self.oc_constraint_label.setStyleSheet(
    ...
    def _check_generate_enabled(self):
        ...
        if hasattr(self, 'generate_btn'):
            self.generate_btn.setEnabled(beta_ok and oc_ok)
    ...
    def export_stl(self):
        ...
        filename, _ = QFileDialog.getSaveFileName(
            self, "Save STL", "vmof_waverider.stl", "STL Files (*.stl)")
        if filename:
            try:
    ...
    def export_tri(self):
        if self.waverider is None:
            ...
            return
        filename, _ = QFileDialog.getSaveFileName(
            self, "Save TRI", "vmof_waverider.tri", "TRI Files (*.tri)")
        if filename:
            try:
    ...
    def export_step(self):
        if self.waverider is None:
            ...
            return
        filename, _ = QFileDialog.getSaveFileName(
            self, "Save STEP", "vmof_waverider.step", "STEP Files (*.step)")
        if not filename:
            return
        try:
...
if __name__ == '__main__':
    ...
    tab = VMOFWaveriderTab()
    tab.setWindowTitle("VMOF Waverider Tab (Standalone Test)")
    tab.resize(1200, 800)
    tab.show()
    sys.exit(app.exec_())
